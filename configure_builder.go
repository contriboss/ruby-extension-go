...
// Build compiles the extension using the ./configure → make workflow
...
func (b *ConfigureBuilder) runConfigure(ctx context.Context, config *BuildConfig, extensionDir, extensionFile string, result *BuildResult) error {
	...
	if config.Verbose {
		result.Output = append(result.Output, fmt.Sprintf("Running: %s %s", configurePath, strings.Join(args, " ")))
...
func (b *ConfigureBuilder) getMakeProgram() string {
	...
	switch runtime.GOOS {
	case "windows":
		return "make" // Most Windows autotools use MinGW/MSYS2 make
	default:
		return "make"
	}
}