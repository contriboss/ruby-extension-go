...
func (b *CargoBuilder) runCargo(ctx context.Context, config *BuildConfig, extensionDir string, result *BuildResult) error {
	...
	if config.Verbose {
		result.Output = append(result.Output, fmt.Sprintf("Running: %s %s", cargoPath, strings.Join(args, " ")))
...
func (b *CargoBuilder) processBuiltExtensions(ctx context.Context, config *BuildConfig, extensionDir string, result *BuildResult) error {
...
func (b *CargoBuilder) getRustcArgs(config *BuildConfig) []string {
	...
	case "windows":
		...
		args = append(args, "-C", "link-arg=-Wl,--dynamicbase")
		args = append(args, "-C", "link-arg=-Wl,--disable-auto-image-base")
		args = append(args, "-C", "link-arg=-static-libgcc")
	}
