...
func (b *RakeBuilder) runMkrfConf(ctx context.Context, config *BuildConfig, extensionDir, extensionFile string, result *BuildResult) error {
	...
	if config.Verbose {
		result.Output = append(result.Output, fmt.Sprintf("Running: %s %s", rubyPath, mkrfPath))
...
func (b *RakeBuilder) runRake(ctx context.Context, config *BuildConfig, extensionDir string, result *BuildResult) error {
	...
	if config.RubyPath != "" {
		...
		// Also set RUBY environment variable
	...
	if config.Verbose {
		result.Output = append(result.Output, fmt.Sprintf("Running: rake %s", strings.Join(args, " ")))
		result.Output = append(result.Output, fmt.Sprintf("Working directory: %s", extensionDir))
	}
